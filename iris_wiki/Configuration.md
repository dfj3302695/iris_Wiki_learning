...
iris.new 方法 返回 iris.Application . 通过 Configure(... iris.Configuration) 设置 和 Run 方法运行。  

另一选项： app.Run/Listen 方法接受一个或多个 iris.Configuration. 每个 iris.Configuration 只是一种 func(app *iris.Application) 的类型。
可以通过修改你的 *iris.Application 来定制 iris.Configuration  

这里有内建 iris.Configurator 的每个核心 Configuration's 字段，例如： iris.WithoutStartupLog,iris.WithCharset("UTF-8"),iris.WithOptimizations and iris.WithConfiguration(iris.Configuration{...})   

view engine, websockets,sessions , each middleware 都有自己的 configuration and options.
